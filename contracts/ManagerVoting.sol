...
contract ManagerVoting is ReentrancyGuard, Transfer, Ownable {
    ...
    // Enum representing types of committees
    enum CommitteeType {
        TokenBased,         // Committee determined by token ownership
        ContributionBased,  // Committee determined by contributions
        PreDefined          // Committee with predefined members
    }
    ...
    struct PoolVotesAllocation {
        mapping(address => uint8) managerVotingPower;
        mapping(address => Status) reviewedByManager;
        uint256 votesFor;
    ...
    mapping(address => PoolVotesAllocation) poolVotesAllocation;
    ...
    /// @notice Emitted when a Pool Committee Type was Changed.
    /// @param poolId The ID of the pool that was updated.
    /// @param newCommittetype New type of Committee.
    event PoolCommitteeTypeChanged(bytes32 indexed poolId, CommitteeType newCommittetype);

    /// @notice Emitted when a Pool thresholdPercentage value was Changed.
    /// @param poolId The ID of the pool that was updated.
    /// @param thresholdPercentage New thresholdPercentage value.
    event PoolThresholdPercentageChanged(bytes32 indexed poolId, uint8 thresholdPercentage);
    ...
    */
    ...
    function registerPool( 
        ...
        // Create a new array in memory that is one element larger than _members
        address[] memory members = new address[](_members.length + 2);

        // Copy the elements from _members to the new array
        for (uint i = 0; i < _members.length; i++) {
            members[i] = _members[i];
        }

        members[members.length -2] = msg.sender;
        members[members.length -1] = address(this);

        profileId = registry.createProfile(_nonce, _name, _metadata, address(this), members);
        profiles.push(profileId);

        _createAndMintPoolAdminHat(msg.sender, profileId);
        _createAndMintPoolManagerHat(members, profileId);

        pool[profileId].isPublic = _isPublic;
        pool[profileId].name = _name;
        pool[profileId].committeeType = _committeeType;
        pool[profileId].votingPowerStatus = VotingPowerStatus.None;
        pool[profileId].thresholdPercentage = thresholdPercentage;
    ...
    function _checkOnlyPoolAdmin(bytes32 _poolID) internal view {
        uint256 adminHatID = pool[_poolID].adminHatID;
        if (!hatsContract.isWearerOfHat(msg.sender, adminHatID)) revert UNAUTHORIZED();
    }

    function setPoolCommitteeType(bytes32 _poolId, CommitteeType _committeeType) external onlyPoolAdmin(_poolId) {
        // Validates that _committeeType is within the range of defined CommitteeType enum values.
        if (uint(_committeeType) > uint(CommitteeType.PreDefined)) {
            revert UNKNOWN_COMMITTEE_TYPE();
        }

        pool[_poolId].committeeType = _committeeType;

        emit PoolCommitteeTypeChanged(_poolId, _committeeType);
    }

    function setPoolThresholdPercentage(bytes32 _poolId, uint8 _thresholdPercentage) external onlyPoolAdmin(_poolId) {
        require(_thresholdPercentage > 0, "Percentage must be greater than zero");
        require(_thresholdPercentage <= 100, "Invalid percentage");

        pool[_poolId].thresholdPercentage = _thresholdPercentage;

        emit PoolThresholdPercentageChanged(_poolId, _thresholdPercentage);
    }

    function initiatePreDefinedVotingPowerAllocation(address[] calldata _managersIds, uint8[] calldata _managerVotingPower, bytes32 _poolId) external onlyPoolAdmin(_poolId) {

        require(pool[_poolId].committeeType == CommitteeType.PreDefined, "Wrong Committee type");
        require(pool[_poolId].votingPowerStatus != VotingPowerStatus.Defined, "Voting power is defined");
        // make sure the arrays are the same length
        if (_managersIds.length != _managerVotingPower.length) revert INVALID();
    ...
    /// @notice Retrieve pool by _poolId
    /// @dev Used when you have the '_poolId' and want to retrieve the pool
    /// @param _poolId The ID of the pool
    /// @return The Pool details for the `_poolId`
    function getPoolById(bytes32 _poolId) external view returns (Pool memory) {
        return pool[_poolId];
    }
    ...
    */
    function supplyProject(bytes32 _projectId, uint256 _amount) external payable nonReentrant {
        ...
        if (pojectSupply[_projectId].has >= pojectSupply[_projectId].need){
            ...
            uint256 alloPool = allo.createPoolWithCustomStrategy{value: msg.value}(
                _projectId,
            ...
            allo.fundPool{value: pojectSupply[_projectId].need}(alloPool, pojectSupply[_projectId].need);
            ...
            allo.registerRecipient(alloPool, encodedRecipientParams);
            projectPool[_projectId] = alloPool;

            emit ProjectPoolCreeated( _projectId, alloPool);
        }
    }
    ...
    */
    ...
    function _createAndMintPoolAdminHat(
        address _hatWearer, 
        bytes32 _poolID
    ) 
        private 
    {
        ...
        hatsContract.mintHat(hat, _hatWearer);
        pool[_poolID].adminHatID = hat;
    }

    function _createAndMintPoolManagerHat(
        address[] memory _hatWearers, 
        bytes32 _poolID
    ) 
        private 
